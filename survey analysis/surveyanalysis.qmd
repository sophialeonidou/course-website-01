...
# Age distribution
data %>%
  mutate(age_group = cut(age, breaks = seq(18, 95, by = 10), include.lowest = TRUE)) %>%
  group_by(age_group) %>%
...
## Employment 
```{r employment-plot}
ggplot(data, aes(x = employment_status, y = loneliness_frequency, fill = employment_status)) +
  geom_boxplot() +
...
##Groupcalls
```{r groupcalls-plot}
ggplot(data, aes(x = group_calls, y = sharing_comfort, fill = group_calls)) +
  geom_boxplot() +
  labs(title = "Group Calls vs. Sharing Comfort Online", x = "Group Calls Participation", y = "Sharing Comfort Level")
```
...
```{r name-plot}
data %>%
  group_by(real_name_usage) %>%
  summarize(mean_sharing_comfort = mean(sharing_comfort, na.rm = TRUE)) %>%
  mutate(percentage = mean_sharing_comfort / sum(mean_sharing_comfort) * 100) %>%
  ggplot(aes(x = "", y = percentage, fill = as.factor(real_name_usage))) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y", start = 0) +
  labs(
    title = "Average Sharing Comfort by Real Name Usage",
    fill = "Real Name Usage Frequency",
    y = "Percentage",
    x = NULL
  ) +
  theme_void() +
